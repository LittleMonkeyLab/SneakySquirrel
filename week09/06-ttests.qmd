...
author: 
  - Remixed by Gordon - Matthew J. C. Crump
aliases: [t-tests.html]
modulecode: 'gordy2test'
date: "22nd July, 2023"
---
...
a mean difference of `r mean(scores)-50`. We found a $t$ =
`r t.test(scores, mu=50)$statistic`. The probability of this $t$ or
larger occurring is $p$ = `r t.test(scores, mu=50)$p.value`. We were
testing the idea that our sample mean of `r mean(scores)` could have
...
`r t.test(scores, mu=50)$p.value`. In other words, chance can do it a
...
We happened to make `r length(save_ps[save_ps<.05])`. The expectation
...
We happened to get `r length(save_ps[save_ps<.05])` simulations where p
was less than .05, that's only `r length(save_ps[save_ps<.05])/1000`
experiments. If you were the researcher, would you want to run an
experiment that would be successful only
`r length(save_ps[save_ps<.05])/1000` of the time? I wouldn't. I would
